...
export const api = {
  ...
  // Candidatos
  listarCandidatos: () => request('/candidato'),
...
// Carrega a eleição aberta, a urna e os candidatos dela.
export async function carregarSessaoUrna() {
  ...
  const todosCandidatos = (await api.listarCandidatos()) || []
  const candidatos = todosCandidatos.filter(
    (c) => !c.eleicao || c.eleicao.id === eleicao.id,
  )
  ...
  return { eleicao, urna, candidatos, aberta }
}
